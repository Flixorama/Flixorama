...
import { setReturnTo } from "./nav_state.js";
...
async function renderCollectionItems(collectionId) {
  ...
  try {
    ...
    items.forEach((item) => {
      ...
      card.addEventListener("click", () => {
        // ✅ clave: details podrá volver aquí usando consumeReturnTo()
        setReturnTo(`collection.html?id=${encodeURIComponent(collectionId)}`);
...
if (backBtn) {
  backBtn.addEventListener("click", () => {
    ...
    // ✅ si estás dentro de una colección → vuelve al listado de colecciones
    if (collectionId) {
